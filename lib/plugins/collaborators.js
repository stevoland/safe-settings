    ...
    if (existing.pendinginvite) {
      ...
      return Promise.resolve([
        new SimpleNoopCommand(
          `Delete Invitation: ${existing.username} has an invitation to ${this.repo} but is not in safe-settings.`
        )
      ])
      ...
      return Promise.resolve([
        new SimpleNoopCommand(
          `Remove Collaborator: ${existing.username} is a collaborator of ${this.repo} but is not in safe-settings.`
        )
      ])
